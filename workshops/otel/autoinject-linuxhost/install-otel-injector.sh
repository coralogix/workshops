...
# Detect Linux distribution and determine package type
PACKAGE_TYPE=""
PACKAGE_NAME=""
INSTALL_CMD=""

# Check for DEB-based systems (Debian, Ubuntu, etc.)
if command -v dpkg >/dev/null 2>&1 && command -v apt-get >/dev/null 2>&1; then
    PACKAGE_TYPE="deb"
    PACKAGE_NAME="opentelemetry-injector.deb"
    INSTALL_CMD="sudo dpkg -i"
# Check for RPM-based systems (RHEL, CentOS, Fedora, etc.)
elif command -v rpm >/dev/null 2>&1 && (command -v yum >/dev/null 2>&1 || command -v dnf >/dev/null 2>&1); then
    PACKAGE_TYPE="rpm"
    PACKAGE_NAME="opentelemetry-injector.rpm"
    INSTALL_CMD="sudo rpm -i"
else
    echo "Error: Unable to detect package management system."
    echo "This script supports DEB-based (Debian, Ubuntu) and RPM-based (RHEL, CentOS, Fedora) systems."
    stop_script
fi
...
echo "Detected system: $PACKAGE_TYPE-based"
...
if [ ! -f "$PACKAGE_NAME" ]; then
    ...
    echo "Please ensure you have built the package using the build script first."
    stop_script
fi
