...
// Sends various metrics to local OTLP collector via gRPC on port 4317
...
class MetricsExample {
  ...
  private setupOpenTelemetry(): void {
    // Create OTLP gRPC exporter pointing to local collector
    const exporter = new OTLPMetricExporter({
      url: 'http://localhost:4317', // Local OTLP collector gRPC endpoint
      headers: {},
    });
    ...
    console.log('Exporting to: http://localhost:4317');
  }
  ...
  public start(): void {
    ...
    console.log('Sending 100 metrics examples to local OTLP collector');
